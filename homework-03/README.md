...
Через Compass создай базу данных `db-contacts` и в ней коллекции `users` и
`contacts`. Возьми [ссылка на json]() и при помощи Compass наполни коллекцию
`contacts` его содержимым.
